<template>
  <div class='border-2 text-center p-1 rounded-xl bg-orange-100' v-if='calculatePages(this.size) < 10 && this.size >= this.pageSize'>
  ...
  <div class='border-2 text-center p-1 rounded-xl bg-orange-100' v-else-if='calculatePages(this.size) > 10 && this.size >= this.pageSize'>
    <PaginationLeft class='bg-white p-1 rounded-md font-bold select-none inline-block align-middle' @click='goLeft()' />
    <ul v-for='(n, i) in calculatePages(this.size)' :key='i' class='inline-block align-middle'>
